...
export class GameController {
    constructor({ characterModel, stageModel, inputController, gameView }) {
        ...
        this.fixedDeltaTime = 1 / 60;       // 물리 연산을 수행할 고정 간격 (60fps)
        this.accumulator = 0;               // 이전 프레임에서 남은 시간을 누적
        this.lastTimestamp = 0;             // 마지막 프레임이 실행된 시간
        this.frameHandle = null;            // requestAnimationFrame의 ID
        this.tick = this.tick.bind(this);   // 루프 함수 바인딩
    }
    ...
    start() {
        ...
        // 스테이지 변화가 없으면 타임스탬프를 초기화하여 루프를 깨끗하게 시작합니다.
    ...
    /**
     * 스테이지의 크기가 변경되었을 때 캐릭터의 위치와 월드 데이터를 보정합니다.
     * @param {object} stageState refresh()에서 반환된 스테이지 상태 정보
     */
