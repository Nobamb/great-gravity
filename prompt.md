...
그리고 보스(boss.webp)도 이번에 게임 화면 오른쪽에 추가를 해주고(boss 기준으로 위랑 왼쪽으로 50%가 보이도록 배치) 게임 화면 최상단의 중앙에는 보스의 hp가 보이도록 만들어줘 디자인은 image/basic-design/stage7.png를 참고해서 만들어줘 그리고 plan.md의 "### 보스 스테이지" 부분의 내용도 같이 참고하면 좋겠어 이미지는 우선 cloudflare의 r2에서 가져올거야 url에 대해서는 내가 따로 env 만들어서 env에 url넣을거니까 cloudflare r2에 대한 환경변수명만 따로 plan.md에 적어줘